...
const OptionCardInventory = () => {
  const dataOptionCard: Array<{ title: string; icon: string; description: string; button: string; enabled: boolean }> = [
    {
      title: "Item Groups",
      icon: "item-groups",
  ...
  return (
    <>
      {dataOptionCard.map((data) => (
        <div className="flex flex-col justify-center items-center drop-shadow-lg bg-white rounded-sm gap-y-2 p-4" key={data.title}>
